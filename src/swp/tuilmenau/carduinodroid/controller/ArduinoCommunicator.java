...
public class ArduinoCommunicator {
    ...
    private BluetoothCommunicatorService bluetoothCommunicator;
    private static byte[] arduinoData = new byte[BUFFERSIZE];
...
//        bluetoothCommunicator = new BluetoothCommunicator(activity);
//        Thread aquireDevicesThread = new Thread(new Runnable(){
//        	public void run(){
//        		findDevice();
//        	} 			
//        }, "Aquiring Devices Thread");
//        aquireDevicesThread.start();
...
//                    Toast.makeText(activity.getBaseContext(), "Arduino Mega 2560 ADK " + "found", Toast.LENGTH_SHORT).show();
        ...
        if (usbDevice == null) {
            if (DEBUG) Log.i(TAG, "No USB-Device found!");
//            Toast.makeText(activity.getBaseContext(), "No Device found", Toast.LENGTH_LONG).show();
            toast("No USB-Device found!");
            activity.startService(new Intent(activity.getBaseContext(), BluetoothCommunicatorService.class));

            
        } else {
            if (DEBUG) Log.i(TAG, "USB-Device found!");
            Intent startUSBIntent = new Intent(activity.getApplicationContext(), USBCommunicatorService.class);
            PendingIntent pendingIntent = PendingIntent.getService(activity.getBaseContext(), 0, startUSBIntent, 0);
    ...
    private byte[] msgBuffer = new byte[BUFFERSIZE];   
    ...
    private void handleReceivedData(Intent intent){
    	...
    	if(msgLength == totalLength){
            ...
            Log.d("Car Data", "msgBuffer: "+ Arrays.toString(msgBuffer));
...
//    	}
    ...
    public void stop(){
    	...
    	if(activity.stopService(new Intent(activity.getBaseContext(),USBCommunicatorService.class))){
	    	 toast(" USB Service closed");
    	}
    	if(activity.stopService(new Intent(activity.getBaseContext(),BluetoothCommunicatorService.class))){
	    	 toast(" Bluetooth Service closed");
    	}
    }
